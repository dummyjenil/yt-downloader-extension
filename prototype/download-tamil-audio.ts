...
function getTamilAudioStreamInfo(videoId: string): {
  title: string
  targetStream: AudioFormatInfo
  allAudioStreams: AudioFormatInfo[]
} {
  console.log(
    `[Extractor] Running yt_dlp format extraction logic for video ID: ${videoId}...`
  )
  ...
  const tamilStream = allAudioStreams.find(
    (s) =>
      (s.langCode === TARGET_LANG ||
        (s.displayName && s.displayName.toLowerCase().includes("tamil"))) &&
      (s.ext === "m4a" || s.itag === 140)
  )

  if (!tamilStream) {
    throw new Error(
      `Tamil audio track ([ta]) not found in extracted formats list.`
    )
  }
...
async function downloadStreamWithHttpFD(
  ...
  console.log(
    `[HttpFD Downloader] Total size: ${(totalBytes / (1024 * 1024)).toFixed(2)} MB (${totalBytes} bytes)`
  )
  ...
  try {
    while (downloadedBytes < totalBytes || totalBytes === 0) {
      const endByte =
        totalBytes > 0
          ? Math.min(downloadedBytes + CHUNK_SIZE - 1, totalBytes - 1)
      ...
      if (!res.ok && res.status !== 206) {
        throw new Error(
          `HTTP Error during chunk fetch [${downloadedBytes}-${endByte}]: ${res.status} ${res.statusText}`
        )
      }
...
async function main() {
  ...
  console.log(
    " YouTube Tamil M4A HD Audio Downloader (yt_dlp TypeScript Replication)"
  )
  console.log(
    "========================================================================="
  )
  ...
  const { title, targetStream, allAudioStreams } =
    getTamilAudioStreamInfo(VIDEO_ID)
  ...
  allAudioStreams.forEach((s) => {
    const isTarget =
      s.langCode === TARGET_LANG ||
      (s.displayName && s.displayName.toLowerCase().includes("tamil"))
        ? " <-- [SELECTED TAMIL TRACK]"
        : ""
    console.log(
      `Format ID: ${s.formatId.padEnd(8)} | Lang: [${s.langCode.padEnd(5)}] | Track: ${s.displayName.padEnd(25)} | Ext: ${s.ext.padEnd(5)} | Size: ${(s.contentLength / (1024 * 1024)).toFixed(2)} MB${isTarget}`
    )
  })
  console.log("-".repeat(80))
  ...
  console.log(
    `  - Language:     [${targetStream.langCode}] ${targetStream.displayName}`
  )
  console.log(`  - Extension:    ${targetStream.ext}`)
  console.log(`  - Codec:        ${targetStream.codec}`)
  console.log(
    `  - Content Size: ${(targetStream.contentLength / (1024 * 1024)).toFixed(2)} MB`
  )
  ...
  await downloadStreamWithHttpFD(
    targetStream.url,
    targetStream.contentLength,
    outputPath
  )
  ...
  console.log(
    `  - File Size on Disk: ${stat.size} bytes (${(stat.size / (1024 * 1024)).toFixed(2)} MB)`
  )
  ...
  if (stat.size > 0 && (ftypMagic === "ftyp" || ftypMagic.includes("M4A"))) {
    ...
    console.log(
      " SUCCESS: Tamil M4A HD audio track downloaded & verified successfully!"
    )
    console.log(
      "========================================================================="
    )
