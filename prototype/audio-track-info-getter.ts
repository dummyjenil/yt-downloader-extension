...
function parseAllAudioStreams(playerResponse: any): AudioStream[] {
  ...
  for (const fmt of allFormats) {
    ...
    const langIdx = knownLangs.includes(langCode)
      ? knownLangs.indexOf(langCode)
      : 0
...
async function main() {
  ...
  console.log(
    `${"ID".padEnd(10)} ${"EXT".padEnd(5)} ${"FILESIZE".padEnd(10)} ${"TBR".padEnd(6)} ${"ACODEC".padEnd(14)} [LANG] AUDIO TRACK`
  )
  console.log("-".repeat(85))
  ...
  streams.forEach((s) => {
    ...
    console.log(
      `${s.fmtId.padEnd(10)} ${s.ext.padEnd(5)} ${sizeStr.padEnd(10)} ${(s.tbr + "k").padEnd(6)} ${s.codec.padEnd(14)} ${langTag} ${s.displayName}${defaultTag}, ${s.qualityNote}`
    )
