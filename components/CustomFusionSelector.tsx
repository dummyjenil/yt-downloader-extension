...
export const CustomFusionSelector: React.FC<CustomFusionSelectorProps> = ({
  ...
  const audioStreamsAll = videoInfo.adaptiveFormats
    .filter((f) => f.mimeType.startsWith("audio/"))
    .sort((a, b) => (b.bitrate || 0) - (a.bitrate || 0))

  const audioStreams = selectedLang
    ? audioStreamsAll.filter(
        (f) =>
          f.langCode === selectedLang || (!f.langCode && selectedLang === "und")
      )
    : audioStreamsAll
  ...
  if (!selectedVideo || !selectedAudio) {
    return (
      <div
        className={`p-4 text-center text-xs ${themeConfig.card} ${themeConfig.radius}`}>
  ...
  const trimmedRatio =
    trimRange && trimRange.enabled && totalSec > 0
      ? Math.max(
          0.005,
          Math.min(
            1.0,
  ...
  const rawTotalSize =
    (parseInt(selectedVideo.contentLength || "0", 10) || 0) +
  ...
  const triggerDownload = () => {
    const chosenSubtitles = captionTracks.filter((t) =>
      selectedSubtitleCodes.includes(t.code)
    )
  ...
  return (
    <div
      className={`flex flex-col gap-4 ${themeConfig.card} ${themeConfig.radius} p-4 shadow-xl my-3`}>
      <div
        className={`flex items-center justify-between pb-3 border-b ${themeConfig.border}`}>
        <span className={`text-sm font-extrabold ${themeConfig.accentText}`}>
          Custom Multi-Stream Fusion
        </span>
        <span
          className={`${themeConfig.badge} uppercase tracking-wider font-bold`}>
          FFmpeg Engine
        </span>
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        <label
          className={`text-xs font-semibold ${themeConfig.mutedText} flex justify-between`}>
          <span>Video Track</span>
          <span className="font-mono">
            {getCodecLabel(selectedVideo.mimeType)}
          </span>
        </label>
      ...
      {/* Audio Language Filter (only when multi-language) */}
      {videoInfo.hasMultiLanguageAudio &&
        videoInfo.audioLanguages &&
        videoInfo.audioLanguages.length > 1 && (
          <div className="flex flex-col gap-1.5">
            <label className={`text-xs font-semibold ${themeConfig.mutedText}`}>
              Audio Language
            </label>
            <div className="relative">
              <select
                ...
                className={`w-full appearance-none ${themeConfig.input} ${themeConfig.radius} pl-3 pr-9 py-2.5 text-xs outline-none cursor-pointer font-sans truncate`}>
                <option value="" className="bg-zinc-900 text-zinc-100 py-1">
                  All Languages
                </option>
      ...
      <div className="flex flex-col gap-1.5">
        <label
          ...
          <span>
            {audioStreams.length}
            {selectedLang ? ` of ${audioStreamsAll.length}` : ""} Available
          </span>
        </label>
        <div className="relative">
          <select
            ...
            {audioStreams.map((stream, idx) => {
              ...
              const langLabel =
                stream.displayName ||
                (stream.langCode ? `[${stream.langCode}]` : "Default Audio")
              return (
                <option
                  key={`${stream.itag}_${stream.langCode || idx}`}
                  value={idx}
                  className="bg-zinc-900 text-zinc-100 py-1">
      ...
      {captionTracks.length > 0 && (
        <div className="flex flex-col gap-2 pt-1">
          <div className="flex justify-between items-center text-xs">
            <span className={`font-semibold ${themeConfig.mutedText}`}>
              Subtitles (Multi-Select):
            </span>
          ...
          <div
            className={`flex flex-col gap-1 max-h-28 overflow-y-auto ${themeConfig.input} ${themeConfig.radius} p-2 no-scrollbar`}>
              ...
              return (
                <label
                  ...
                  className={`flex items-center gap-2 px-2.5 py-1.5 ${themeConfig.radius} text-xs cursor-pointer select-none transition-colors ${
                    isChecked
                      ? `${themeConfig.badge} font-bold`
                      : `${themeConfig.mutedText} hover:bg-white/5`
                  }`}>
                  <input
                    type="checkbox"
                    checked={isChecked}
      ...
      {!containersMatch && (
        <div className="text-xs text-amber-300 bg-amber-500/10 border border-amber-500/30 rounded-xl p-3 leading-relaxed">
          ⚠️ <strong>Format Mismatch:</strong>{" "}
          {getFormatLabel(selectedVideo.mimeType)} video +{" "}
          {getFormatLabel(selectedAudio.mimeType)} audio. Merging matching
          formats is faster.
        </div>
      )}
      ...
      <div
        className={`border ${themeConfig.border} ${themeConfig.radius} p-3 flex items-center justify-between`}>
        <div className="flex flex-col">
          <span className={`text-[10px] ${themeConfig.mutedText} font-medium`}>
            Est. File Size
          </span>
          <span className={`text-sm font-bold ${themeConfig.accentText}`}>
