...
export const PopupDashboardTab: React.FC<PopupDashboardTabProps> = ({
  activeDownloads
}) => {
      ...
      {activeDownloads.length === 0 ? (
        ...
        <div className="flex flex-col gap-3 max-h-[460px] overflow-y-auto no-scrollbar">
          {activeDownloads.map((job) => (
            <div
              key={job.id}
              className={`${themeConfig.card} ${themeConfig.radius} p-3.5 shadow-lg`}>
              ...
              <div
                className={`flex justify-between text-xs ${themeConfig.mutedText} mt-1 font-medium`}>
                <span>
                  {job.status === "paused"
                    ? "Paused"
