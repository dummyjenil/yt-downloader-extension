...
export const StreamRow: React.FC<StreamRowProps> = ({
  ...
  return (
    <div
      className={`flex items-center justify-between p-3.5 ${themeConfig.radius} ${themeConfig.card} transition-all`}>
      <div className="flex flex-col gap-1 flex-1 min-w-0 pr-3">
        <span className="text-sm font-bold truncate">{label}</span>
        <span className={`text-xs ${themeConfig.mutedText} truncate`}>
          {meta}
        </span>
      </div>
      <button
