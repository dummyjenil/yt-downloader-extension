import {
  getClientConfig,
  getClientFallbackSequence
} from "~/utils/youtube-clients"
...
let cachedApiKey: string | null =
  process.env.PLASMO_PUBLIC_YOUTUBE_API_KEY || null
...
export async function fetchVideoInfo(videoId: string) {
  ...
  // Read preferred client setting from chrome.storage.local
  const storageRes = await new Promise<{ preferred_client?: string }>(
    (resolve) => {
      if (
        ...
        chrome.storage.local.get(["preferred_client"], (res) =>
          resolve(res || {})
        )
      } else {
        resolve({})
  ...
  const preferredClientKey = storageRes.preferred_client || "AUTO"
  const clientKeys = getClientFallbackSequence(preferredClientKey)

  const clientConfigs = clientKeys.map((key) => {
    const config = getClientConfig(key, visitorData || undefined)
    return {
      name: key,
      apiKey: apiKey,
      payloadContext: {
        client: config.payloadClient
      }
    }
  })
