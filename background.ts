...
function saveToHistory(entry: any) {
  chrome.storage.local.get({ downloadHistory: [] }, (result) => {
    ...
    const updated = [
      entry,
      ...history.filter((h: any) => h.id !== entry.id)
    ].slice(0, 100)
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "OPEN_PLAYLIST_TAB") {
    const url = chrome.runtime.getURL(
      `tabs/playlist.html?list=${message.playlistId}`
    )
  ...
  if (
    message.type === "PAUSE_DOWNLOAD" ||
    message.type === "RESUME_DOWNLOAD" ||
    message.type === "CANCEL_DOWNLOAD"
  ) {
    const targetUrl = chrome.runtime.getURL("tabs/download.html")
    chrome.tabs.query({}, (tabs) => {
      const existingTab = tabs.find((t) => t.url && t.url.startsWith(targetUrl))
  ...
  if (
    message.type === "TAB_DOWNLOAD_FAILED" ||
    message.type === "TAB_DOWNLOAD_CANCELLED"
  ) {
    const { id } = message
  ...
  if (
    message.type === "GET_ACTIVE_DOWNLOADS" ||
    message.type === "GET_ALL_DOWNLOADS"
  ) {
