...
export async function setupDestinationStream(
  ...
  if (saveMode === "directory") {
    try {
      ...
      console.warn("Directory handle write failed or stale, falling back to browser anchor stream:", err);
    }
  }

  // 2. Browser Anchor Stream fallback (or default when saveMode === "browser")
  if (!writableStream) {
    console.log(`Using Browser Anchor Stream for download (${saveMode} mode):`, job.title);
