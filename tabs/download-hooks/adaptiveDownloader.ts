...
import { mergeChunksToBuffer, runFFmpegMerge } from "./ffmpegMerge"
...
export async function processAdaptiveDownload(
  ...
  if (job.trimRange && job.trimRange.enabled) {
    try {
      ...
      if (!vSidx || !aSidx) {
        console.log(
          "🔍 [processAdaptiveDownload] Resolving Video & Audio SIDX in parallel..."
        )
        const [vRes, aRes] = await Promise.all([
          fetchSidxByteRange(
            job.url,
            job.trimRange.startTimeSec,
            job.trimRange.endTimeSec,
            job.initRange,
            job.indexRange
          ),
          fetchSidxByteRange(
      ...
      if (vSidx && !aSidx && job.audioSize && job.audioSize > 0) {
        const estDuration =
          vSidx.totalDurationSec || Math.max(job.trimRange.endTimeSec, 180)
        const aStart = Math.floor(
          job.audioSize * (job.trimRange.startTimeSec / estDuration)
        )
        const aEnd = Math.min(
          job.audioSize - 1,
          Math.ceil(job.audioSize * (job.trimRange.endTimeSec / estDuration))
        )
        console.log(
          `💡 [processAdaptiveDownload] Audio SIDX missing. Using proportional audio range fallback: ${aStart}-${aEnd} (${aEnd - aStart + 1} bytes) based on stream duration ${estDuration}s`
        )
        aSidx = {
      ...
      if (vSidx && aSidx) {
        ...
        const updateSidxProgress = (addedBytes: number) => {
          ...
          job.percent = Math.min(
            99,
            Math.round((sidxDownloaded / sidxTotalPayload) * 100)
          )
          ...
          if (job.speedHistory.length > 1) {
            ...
            job.speed =
              elapsed > 0 ? (sidxDownloaded - first.bytes) / elapsed : 0
          }
        ...
        const videoBuf = new Uint8Array(
          vSidx.initBytes.byteLength + vSubstream.byteLength
        )
        videoBuf.set(vSidx.initBytes, 0)
        videoBuf.set(vSubstream, vSidx.initBytes.byteLength)

        const audioBuf = new Uint8Array(
          aSidx.initBytes.byteLength + aSubstream.byteLength
        )
        ...
        const vSeek = Math.max(
          0,
          job.trimRange.startTimeSec - vSidx.subsegmentStartSec
        )
        const aSeek = Math.max(
          0,
          job.trimRange.startTimeSec - aSidx.subsegmentStartSec
        )
        ...
        const subtitleBuffers = await fetchSubtitleBuffers(
          job.selectedSubtitles,
          job.trimRange
        )
        ...
        console.log(
          "💾 [processAdaptiveDownload Writing merged output to disk]",
          {
        ...
        console.log(
          "🎉 [processAdaptiveDownload COMPLETE] Saved file size:",
        ...
        console.warn(
          "⚠️ [processAdaptiveDownload SIDX MATCH FAILED] Falling back to full stream adaptive trim..."
        )
      }
    } catch (sidxErr) {
      console.warn(
        "⚠️ [processAdaptiveDownload SIDX ERROR] Falling back to full stream trim:",
        sidxErr
      )
    }
  }
  ...
  console.log(
    "📦 [processAdaptiveDownload SECTION 2] Full stream download fallback..."
  )
  ...
  if (!job.adaptiveVideoChunks)
    job.adaptiveVideoChunks = new Map<number, ArrayBuffer>()
  if (!job.adaptiveAudioChunks)
    job.adaptiveAudioChunks = new Map<number, ArrayBuffer>()
  ...
  const updateProgress = () => {
    ...
    if (job.speedHistory.length > 1) {
      ...
      const elapsed = (now - first.time) / 1000
      job.speed =
        elapsed > 0 ? (job.downloadedBytes - first.bytes) / elapsed : 0
    } else {
  ...
  const downloadVideo = () => {
    return new Promise<void>((resolve, reject) => {
      const fetchNext = () => {
        ...
        while (
          activeVideoFetches.size < currentConcurrency &&
          job.launchedVideoChunks! < totalVideoChunks
        ) {
            ...
            .then((arrayBuffer) => {
              ...
              if (!arrayBuffer) {
                if (!job.paused && !job.cancelled) {
                  reject(
                    new Error(
                      `Failed to download video chunk ${chunkIdx + 1}/${totalVideoChunks}`
                    )
                  )
                }
                return
  ...
  const downloadAudio = () => {
    return new Promise<void>((resolve, reject) => {
      const fetchNext = () => {
        ...
        while (
          activeAudioFetches.size < currentConcurrency &&
          job.launchedAudioChunks! < totalAudioChunks
        ) {
            ...
            .then((arrayBuffer) => {
              ...
              if (!arrayBuffer) {
                if (!job.paused && !job.cancelled) {
                  reject(
                    new Error(
                      `Failed to download audio chunk ${chunkIdx + 1}/${totalAudioChunks}`
                    )
                  )
                }
                return
  ...
  try {
    ...
    const videoBuf = mergeChunksToBuffer(
      job.adaptiveVideoChunks!,
      totalVideoChunks
    )
    const audioBuf = mergeChunksToBuffer(
      job.adaptiveAudioChunks!,
      totalAudioChunks
    )
