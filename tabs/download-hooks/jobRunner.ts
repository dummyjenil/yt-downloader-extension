...
export async function setupAndStartJob(
  ...
  try {
        ...
        chrome.runtime.sendMessage(
          ...
          (response) => {
            if (chrome.runtime.lastError) {
              ...
              reject(
                new Error(response?.error || "Failed to fetch stream details.")
              )
            }
          }
        )
    ...
    const writableStream = await setupDestinationStream(
      job,
      setDirPermission,
      currentSaveMode
    )
    const fetchStreamSize = async (streamUrl: string): Promise<number> => {
      ...
      const headResponse = await fetch(`${streamUrl}${sep}ext_download=true`, {
        method: "HEAD"
      }).catch(() => null)
      ...
      const rangeResponse = await fetch(
        `${streamUrl}${sep}range=0-0&ext_download=true`
      ).catch(() => null)
    ...
    let videoSize =
      job.audioUrl && job.audioSize
        ? Math.max(0, job.totalSize - job.audioSize)
        : job.totalSize
    if (!videoSize) {
