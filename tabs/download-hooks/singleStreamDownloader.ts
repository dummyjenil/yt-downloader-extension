...
import { downloadRangeInParallel, fetchChunkWithRetry } from "./chunkDownloader"
...
export async function processSingleStreamDownload(
      ...
      if (sidx) {
        ...
        const updateSidxProgress = (addedBytes: number) => {
          ...
          job.percent = Math.min(
            99,
            Math.round((sidxDownloaded / rangeSize) * 100)
          )
        ...
        const fullBuf = new Uint8Array(
          sidx.initBytes.byteLength + substream.byteLength
        )
        ...
        const vSeek = Math.max(
          0,
          job.trimRange.startTimeSec - sidx.subsegmentStartSec
        )
        ...
        console.log("🎉 [processSingleStreamDownload COMPLETE]", {
          savedSize: trimmedBuf.byteLength
        })
        ...
        console.warn(
          "⚠️ [processSingleStreamDownload SIDX MATCH FAILED] Falling back to full stream trim..."
        )
      }
    } catch (sidxErr) {
      console.warn(
        "⚠️ [processSingleStreamDownload SIDX ERROR] Falling back to full stream trim:",
        sidxErr
      )
    }
  }
  ...
  console.log(
    "📦 [processSingleStreamDownload SECTION 2] Full stream download fallback..."
  )
  ...
  const downloadLoop = async () => {
    ...
    while (
      job.activeFetches.size < currentConcurrency &&
      job.launchedChunks < totalChunks
    ) {
      ...
      fetchChunkWithRetry(job, chunkIdx, totalChunks, currentChunkSize)
        .then(async (arrayBuffer) => {
          ...
          if (requiresPostProcessing) {
            ...
            job.percent = Math.round(
              (job.downloadedBytes / job.totalSize) * 100
            )
            ...
            if (
              job.downloadedChunks.size === totalChunks &&
              job.status === "downloading"
            ) {
              job.status = "processing" as any
              const fullBuf = mergeChunksToBuffer(
                job.downloadedChunks,
                totalChunks
              )
          ...
          if (
            job.nextChunkToWrite >= totalChunks &&
            job.status === "downloading"
          ) {
            job.status = "complete"
            job.percent = 100
