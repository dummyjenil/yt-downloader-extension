...
export const ActiveDownloadsTab: React.FC<ActiveDownloadsTabProps> = ({
  ...
  return (
    <div>
      ...
      {!defaultDirName && (
        <div className="flex items-center justify-between gap-4 p-5 bg-blue-500/10 border border-blue-500/30 rounded-2xl mb-6 text-sm text-blue-200 leading-relaxed">
          <div className="flex items-center gap-3">
            ...
            <span>
              Default download folder is not set. Downloads will wait as <strong>Idle</strong>. Click <strong>Start</strong> on the job card to select a save location manually, or select a default folder in <strong>Settings</strong> for automatic downloads.
            </span>
          </div>
        </div>
      )}
      ...
      {jobList.length === 0 ? (
        <div className={`text-center py-16 px-6 ${themeConfig.card} ${themeConfig.radius} border-dashed ${themeConfig.mutedText}`}>
          <svg
            width="52"
            height="52"
            viewBox="0 0 24 24"
          ...
          <p className="m-0 text-base font-bold text-zinc-100">No active downloads running</p>
          <p className="mt-1 text-sm">Start downloading from a YouTube page to see progress here</p>
        </div>
      ) : (
            ...
            >
              <div className="flex justify-between items-start gap-4 mb-3">
                <div>
                  ...
                  <span className={`text-xs ${themeConfig.mutedText} mt-1 inline-block font-medium`}>
              ...
              {/* Progress details */}
              <div className={`flex justify-between text-xs ${themeConfig.mutedText} mb-2 font-medium`}>
                <span>
                  {job.status === "paused"
                    ? "Paused"
                    : job.status === "error"
              ...
              {/* Progress Bar */}
              <div className={`w-full h-2.5 bg-white/5 ${themeConfig.radius} overflow-hidden mb-3 border ${themeConfig.border}`}>
                <div
              ...
              {job.status === "downloading" && (
                <div className={`flex gap-5 text-xs ${themeConfig.mutedText} font-medium`}>
                  <span>Speed: <strong className="text-zinc-100">{formatBytes(job.speed)}/s</strong></span>
                  <span>ETA: <strong className="text-zinc-100">{formatTime(job.eta)}</strong></span>
                </div>
              )}
            </div>
          ))}
