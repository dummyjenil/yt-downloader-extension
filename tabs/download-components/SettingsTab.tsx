...
import { YOUTUBE_CLIENT_CONFIGS } from "../../utils/youtube-clients"
...
export const SettingsTab: React.FC<SettingsTabProps> = ({
      ...
      if (installed) {
        ...
        if (
          ...
          chrome.storage.local.get(
            ["ffmpeg_status", "ffmpeg_progress", "ffmpeg_error"],
            (res) => {
  ...
  return (
    <div
      className={`${themeConfig.card} ${themeConfig.radius} p-8 ${themeConfig.shadow}`}>
      ...
      <div className="mb-7">
        ...
        <div className="flex flex-col gap-2 max-w-lg">
          <label
            className={`flex items-start gap-3 p-3 ${themeConfig.input} ${themeConfig.radius} cursor-pointer select-none`}>
            <input
              type="radio"
              name="saveMode"
              value="directory"
            ...
            <div className="flex flex-col">
              <span className="text-xs font-bold">
                1. Direct Folder Save (File System API)
              </span>
              <span className={`text-[11px] ${themeConfig.mutedText} mt-0.5`}>
                Saves directly into your selected custom folder without opening
                save popups.
              </span>
            </div>
          </label>
          ...
          <label
            ...
            <div className="flex flex-col">
              <span className="text-xs font-bold">
                2. Browser Anchor Download (Chrome Downloads)
              </span>
              <span className={`text-[11px] ${themeConfig.mutedText} mt-0.5`}>
                Downloads directly via Chrome's download manager into your
                default Downloads folder without prompt popups.
              </span>
            </div>
          </label>
        </div>
      ...
      {/* YouTube Extractor Client Picker */}
      <div className="mb-7">
        <label className="block text-sm font-bold mb-2">
          YouTube Extractor Client (yt-dlp Engine Specs)
        </label>
        <p className={`text-xs ${themeConfig.mutedText} mb-3 leading-relaxed`}>
          Select which YouTube InnerTube client identity is tried first when
          extracting video stream URLs. If the primary client fails, the
          extension automatically falls back to alternative clients.
        </p>
        <select
          value={preferredClient}
          onChange={(e) => updateSetting("preferred_client", e.target.value)}
          className={`${themeConfig.input} ${themeConfig.radius} p-3 text-sm w-full max-w-md outline-none font-bold cursor-pointer`}>
          <option value="AUTO" className="bg-zinc-900 text-zinc-100 py-1">
            Auto / Smart Sequence (Recommended: Android VR ➔ TV ➔ iOS ➔ Android
            ➔ Mobile Web ➔ Desktop Web)
          </option>
          {Object.values(YOUTUBE_CLIENT_CONFIGS).map((client) => (
            <option
              key={client.key}
              value={client.key}
              className="bg-zinc-900 text-zinc-100 py-1">
              {client.label} — {client.description}
            </option>
          ))}
        </select>
      </div>
      ...
      {/* Visual Theme Picker */}
      <div className="mb-7">
        <label className="block text-sm font-bold mb-2">Visual UI Theme</label>
        <p className={`text-xs ${themeConfig.mutedText} mb-3 leading-relaxed`}>
          Customize the aesthetic theme across the extension popup and full
          dashboard page.
        </p>
        <select
          value={theme}
      ...
      <div className="mb-7">
        ...
        {defaultDirName && (
          <div
            className={`mt-3 text-xs ${themeConfig.accentText} font-semibold`}>
      ...
      <div className="mb-7">
        ...
        <p className={`text-xs ${themeConfig.mutedText} mb-3 leading-relaxed`}>
          Adjusting this can improve speed depending on your connection. Larger
          chunk size uses slightly more memory buffering but makes fewer network
          requests.
        </p>
        <select
          value={chunkSize}
          onChange={(e) =>
            updateSetting("chunkSize", parseInt(e.target.value, 10))
          }
      ...
      <div className="mb-7">
        ...
        <p className={`text-xs ${themeConfig.mutedText} mb-3 leading-relaxed`}>
          Number of chunks to fetch simultaneously. Higher values speed up
          download but can trigger YouTube rate-limiting.
        </p>
        <select
          value={concurrency}
          onChange={(e) =>
            updateSetting("concurrency", parseInt(e.target.value, 10))
          }
          className={`${themeConfig.input} ${themeConfig.radius} p-3 text-sm w-full max-w-xs outline-none font-bold cursor-pointer`}>
          ...
          <option value={3} className="bg-zinc-900 text-zinc-100">
            3 Parallel Chunks (Recommended)
          </option>
          <option value={5} className="bg-zinc-900 text-zinc-100">
            5 Parallel Chunks (Fast)
          </option>
          <option value={8} className="bg-zinc-900 text-zinc-100">
            8 Parallel Chunks (Aggressive)
          </option>
        </select>
      </div>
      ...
      <div className="mb-7">
        ...
        <p className={`text-xs ${themeConfig.mutedText} mb-3 leading-relaxed`}>
          Control how many videos in a playlist download in parallel. Other
          videos will wait in queue automatically.
        </p>
        <select
          value={maxConcurrentJobs}
          onChange={(e) =>
            updateSetting("maxConcurrentJobs", parseInt(e.target.value, 10))
          }
          className={`${themeConfig.input} ${themeConfig.radius} p-3 text-sm w-full max-w-xs outline-none font-bold cursor-pointer`}>
      ...
      <div className="mb-3">
        ...
        <p className={`text-xs ${themeConfig.mutedText} mb-3 leading-relaxed`}>
          FFmpeg is used at runtime to fuse high-definition adaptive video with
          audio tracks. To avoid bulkier extension packages, it is downloaded
          and installed locally on demand.
        </p>

        <div
          className={`flex flex-col gap-3 ${themeConfig.card} ${themeConfig.radius} p-4 border ${themeConfig.border}`}>
          <div className="flex justify-between items-center">
            <span className={`text-xs ${themeConfig.mutedText} font-semibold`}>
              Status:
            </span>
            <span
