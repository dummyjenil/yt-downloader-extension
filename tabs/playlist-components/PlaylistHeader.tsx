...
export const PlaylistHeader: React.FC<PlaylistHeaderProps> = ({
  ...
  return (
    <div className={`flex flex-col gap-5 ${themeConfig.card} ${themeConfig.radius} p-6 shadow-2xl mb-6 border ${themeConfig.border}`}>
      {/* Playlist Meta Header */}
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 pb-4">
        <div>
          <span className={`text-xs uppercase tracking-wider font-extrabold ${themeConfig.accentText}`}>
            Playlist Batch Downloader
          </span>
          <h1 className="text-xl sm:text-2xl font-black m-0 mt-1 leading-snug">
            {playlistTitle || "YouTube Playlist"}
          </h1>
          <p className={`text-xs sm:text-sm ${themeConfig.mutedText} font-semibold m-0 mt-1`}>
            {playlistAuthor ? `by ${playlistAuthor} • ` : ""}{videoCount} Videos total
          </p>
        </div>

        {/* Directory Selector Area */}
        <div className={`flex items-center gap-3 ${themeConfig.navContainer} ${themeConfig.radius} p-2.5 px-4`}>
          <div className="flex flex-col">
            <span className={`text-[10px] uppercase font-bold ${themeConfig.mutedText}`}>
              Target Save Folder
            </span>
            <span className="text-xs font-bold truncate max-w-[200px]">
              {defaultDirName && dirPermission === "granted" ? `📁 ${defaultDirName}` : "⚠️ No folder selected"}
            </span>
          </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <option value="1080p" className="bg-zinc-900 text-zinc-100">1080p Full HD MP4</option>
