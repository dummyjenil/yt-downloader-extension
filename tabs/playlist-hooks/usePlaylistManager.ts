...
export function usePlaylistManager() {
  ...
  const [globalPreset, setGlobalPreset] = useState<"1080p" | "720p" | "480p" | "audio">("1080p");
  ...
  const fetchPlaylist = (id: string) => {
    ...
    if (typeof chrome !== "undefined" && chrome.runtime) {
      chrome.runtime.sendMessage({ type: "GET_PLAYLIST_DETAILS", playlistId: id }, (res) => {
        setLoading(false);
        if (chrome.runtime.lastError) {
          ...
          const initialItems: ConfiguredPlaylistItem[] = res.details.videos.map((v: PlaylistVideoItem) => ({
            ...v,
            selected: true,
            formatOption: "1080p",
            embedThumbnail: true,
  ...
  const updateItem = (videoId: string, updates: Partial<ConfiguredPlaylistItem>) => {
    setItems((prev) => prev.map((item) => (item.videoId === videoId ? { ...item, ...updates } : item)));
  };
  ...
  const startBatchDownload = async () => {
    if (!defaultDirName || dirPermission !== "granted") {
      alert("Please select a target destination folder before starting batch download.");
    ...
    for (const item of selectedItems) {
      ...
      try {
        const infoRes = await new Promise<any>((resolve, reject) => {
          chrome.runtime.sendMessage({ type: "GET_VIDEO_INFO", videoId: item.videoId }, (r) => {
            if (r && r.success) resolve(r.info);
            else reject(new Error(r?.error || "Failed to fetch stream details"));
          });
        });
        ...
        if (item.formatOption === "audio") {
          chosenStream = adaptiveFormats.find((f: any) => f.mimeType.startsWith("audio/"));
        } else {
          // Find matching resolution
          chosenStream = formats.find((f: any) => f.qualityLabel?.includes(item.formatOption)) ||
                         adaptiveFormats.find((f: any) => f.mimeType.startsWith("video/") && f.qualityLabel?.includes(item.formatOption)) ||
                         formats[0] || adaptiveFormats[0];

          if (chosenStream?.mimeType?.startsWith("video/")) {
            chosenAudio = adaptiveFormats.find((f: any) => f.mimeType.startsWith("audio/"));
          }
        }
