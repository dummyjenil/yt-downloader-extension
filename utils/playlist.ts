...
export async function fetchPlaylistDetails(playlistId: string): Promise<PlaylistDetails> {
  ...
  for (const item of sidebar) {
    ...
    if (primary && primary.title) {
      playlistTitle = primary.title.runs?.[0]?.text || primary.title.simpleText || playlistTitle;
    }
    const secondary = item?.playlistSidebarSecondaryInfoRenderer;
    if (secondary && secondary.owner) {
      playlistAuthor = secondary.owner.videoOwnerRenderer?.title?.runs?.[0]?.text || playlistAuthor;
    }
  }
  ...
  for (const tab of tabs) {
    ...
    for (const section of contents) {
      ...
      for (const item of items) {
        ...
        const title = renderer.title?.runs?.[0]?.text || renderer.title?.simpleText || "YouTube Video";
        const author = renderer.shortBylineText?.runs?.[0]?.text || playlistAuthor;
        const lengthSeconds = renderer.lengthSeconds || "0";
        const thumbnail = renderer.thumbnail?.thumbnails?.slice(-1)[0]?.url || `https://img.youtube.com/vi/${videoId}/hqdefault.jpg`;
