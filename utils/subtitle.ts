...
export function jsonToWordSrt(
  ...
  const startMsLimit = isTrimmed
    ? Math.max(0, trimRange.startTimeSec * 1000)
    : 0
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    for (let k = 0; k < words.length; k++) {
      ...
      const finalStart = isTrimmed
        ? Math.max(0, start_ms - startMsLimit)
        : start_ms
...
export async function fetchSubtitleBuffers(
  ...
  if (selectedSubtitles && selectedSubtitles.length > 0) {
    for (let i = 0; i < selectedSubtitles.length; i++) {
      ...
      let jsonUrl = track.baseUrl.includes("fmt=")
