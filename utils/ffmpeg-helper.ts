...
export async function saveCachedFile(key: string, blob: Blob, version: string): Promise<void> {
  const db = await openDb();
  return new Promise((resolve, reject) => {
...
export async function isFFmpegInstalled(expectedVersion: string = FFMPEG_VERSION): Promise<boolean> {
  try {
    const db = await openDb();
    const checkFile = (key: string): Promise<boolean> => {
      return new Promise((resolve) => {
        ...
        request.onsuccess = () => {
          ...
          resolve(val && val.blob instanceof Blob && val.version === expectedVersion);
        };
...
export async function loadFFmpegFromCache(
  ...
  if (!jsBlob || !wasmBlob) {
    throw new Error("FFmpeg core files not found in IndexedDB. Please install FFmpeg first.");
  }
