...
export function extractChapters(
          ...
          if (chaptersData.length > 0) {
            ...
            for (let i = 0; i < chaptersData.length; i++) {
              ...
              if (ch) {
                ...
                const nextStartMs =
                  i < chaptersData.length - 1
                    ? chaptersData[i + 1]?.chapterRenderer
                        ?.timeRangeStartMillis || totalDurationSec * 1000
  ...
  for (let i = 0; i < parsedItems.length; i++) {
    ...
    const nextStartMs =
      i < parsedItems.length - 1 ? parsedItems[i + 1].startMs : totalMs
