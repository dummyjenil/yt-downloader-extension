...
export const ActiveDownloads: React.FC<ActiveDownloadsProps> = ({
  ...
  return (
    <div className={`mt-6 pt-5 border-t ${themeConfig.border} font-sans`}>
      <h4
        className={`m-0 mb-3.5 text-xs font-extrabold ${themeConfig.accentText} uppercase tracking-wider`}>
      ...
      <div className="flex flex-col gap-3 max-h-48 overflow-y-auto no-scrollbar">
        {activeJobs.map((job) => (
          <div
            key={job.id}
            className={`${themeConfig.card} ${themeConfig.radius} p-3.5 shadow-md`}>
            ...
            <div
              className={`w-full bg-white/5 rounded-full h-2 overflow-hidden my-2 border ${themeConfig.border}`}>
              <div
            ...
            <div
              className={`flex justify-between text-xs ${themeConfig.mutedText} font-semibold`}>
              <span>
                {job.status === "paused"
                  ? "Paused"
