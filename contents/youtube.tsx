...
function YoutubeOverlayContent() {
  ...
  const [activeTab, setActiveTab] = useState<
    "video" | "audio" | "adaptive" | "fusion" | "subtitle"
  >("video")
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        ...
        if (chrome.runtime.lastError) {
          ...
          setError(
            response?.error || "Unable to extract stream links for this video."
          )
        }
      }
    )
    ...
    if (typeof chrome !== "undefined" && chrome.runtime) {
      chrome.runtime
        .sendMessage({
          ...
          url: stream.url,
          videoId: (videoInfo as any).videoId || videoId,
          title: filename,
          ext: ext,
  ...
  const cleanTitleForMatch = videoInfo
    ? videoInfo.title.replace(/[\\/:*?"<>|]/g, "_")
    : ""
  const currentVideoJob = activeJobs.find(
    (d) =>
      d.title.includes(cleanTitleForMatch) ||
      cleanTitleForMatch.includes(d.title)
  )

  const isCurrentlyDownloading = currentVideoJob !== undefined
  const currentDownloadPercent = currentVideoJob
    ? currentVideoJob.percent
    : null
  ...
  return (
    <div className="font-sans text-base">
      ...
      {showDialog && (
        <div
          className="fixed inset-0 bg-black/75 backdrop-blur-md flex items-center justify-center z-[99999999] animate-fadeIn p-4 sm:p-5 overflow-y-auto"
          onClick={() => setShowDialog(false)}>
          <div
            className={`w-full max-w-[600px] max-h-[85vh] my-auto ${themeConfig.container} ${themeConfig.radius} shadow-2xl p-5 sm:p-6 flex flex-col box-border overflow-y-auto animate-slideUp border-2 ${themeConfig.border}`}
            onClick={(e) => e.stopPropagation()}>
            {/* Header */}
            <div
              className={`flex justify-between items-start mb-4 pb-3 border-b ${themeConfig.border} shrink-0`}>
              <div className="flex-1 pr-4">
                ...
                <p
                  className={`text-xs sm:text-sm ${themeConfig.mutedText} m-0 font-bold`}>
                  {videoInfo
                    ? `by ${videoInfo.author}`
                    : playlistId
                      ? "Playlist batch downloader ready"
                      : "Please wait..."}
                </p>
              </div>
              <button
            ...
            {playlistId && (
              <div
                className={`p-4 mb-4 bg-gradient-to-r from-violet-600/30 to-fuchsia-600/30 border border-violet-500/40 ${themeConfig.radius} flex items-center justify-between gap-3 shadow-lg shrink-0`}>
                <div className="flex flex-col">
                  ...
                  <span className="text-xs sm:text-sm font-bold text-white mt-0.5">
                    Batch Download entire playlist with custom video quality &
                    audio settings
                  </span>
                </div>
                <button
            ...
            {/* Detail Card (Video) */}
            {videoInfo && <VideoDetailCard videoInfo={videoInfo} />}

            {/* Loader State */}
            {loading && (
              <div
                className={`flex flex-col items-center justify-center py-12 ${themeConfig.mutedText} text-base font-bold`}>
